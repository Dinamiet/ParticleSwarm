...
{
	swarm->RandomFunction    = randomFunction;
	...
	for (size_t i = 0; i < numParticles; ++i) { Particle_Init(&swarm->Particles[i], randomFunction, lowerBound, upperBound); }
}
...
void Swarm_Optimize(Swarm* swarm, Objective evaluate, size_t maxIterations, float w, float c1, float c2)
{
	for (size_t itt = 0; itt < maxIterations; itt++)
	{
		...
		for (size_t i = 0; i < swarm->NumParticles; i++)
		{
			Particle_Update(&swarm->Particles[i], swarm->GlobalBestPosition, swarm->RandomFunction, w, c1, c2);
		}
	}
}
